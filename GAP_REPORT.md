...
Queries where iDudes is NOT being cited but probably SHOULD be, given existing portfolio content.

Format per gap:
- **Query**: exact probe string
- **Who's cited instead**: top competitor / source the engine picked
- **iDudes asset that should rank**: property + ideal post (existing or needed)
- **Hypothesis**: why we're not being cited yet
